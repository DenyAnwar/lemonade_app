...
class MainActivity : AppCompatActivity() {
    ...
    private fun clickLemonImage() {
        ...
        when (lemonadeState) {
            SELECT -> {
                // if the state is SELECT that means on tap we will try to squeeze the lemon
                // so set the state as SQUEEZE.
                lemonadeState = SQUEEZE

                // determining the size of tree by pick() function
                val tree: LemonTree = lemonTree
                lemonSize = tree.pick()

                // in starting there is no squeeze so count=0, 0, after the count will be
                // increased by 1.
                squeezeCount = 0
            }
            SQUEEZE -> {
                // now we are squeezing so count will be increase by 1 and size will be
                // decreased by 1.
                squeezeCount += 1
                lemonSize -= 1

                // now the lemonadeState will in Squeeze state until we do not complete the
                // whole lemon(lemonSize=0).
                // so if the lemonSize is 0, the the state will be DRINK till then we have
                // to Squeeze.
            ...
            // if the state is Drink the we will just drink the lemonade and state will be restart.
            DRINK -> {
                lemonadeState = RESTART
                lemonSize = -1
            }
            // restart mean now we will have to select the lemon from the tree.
            RESTART -> { lemonadeState = SELECT }
        }
        // as per result of the above we will call the setViewElements() function and
